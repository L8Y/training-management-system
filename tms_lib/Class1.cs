...
namespace tms_lib
{
    public class Class1
    {
        ...
        public bool login(string email, string password, string role)
        {
            ...
            if (role == "hr")
            {
                isValidUser = data.Hrs.Where(c => c.EmailId == email && c.Password == password).Count() > 0;

                //var hr_id = from h in ob.Hrs
                //where h.EmailId == email && h.Password == password
                //select  h.Id;
                //TempData["login_id"] = hr_id;
                //TempData.Keep();
                //HttpContext.Session.SetString("login", hr_id);
            }
            else if (role == "manager")
            {
                isValidUser = data.Managers.Any(c => c.EmailId == email && c.Password == password);
            }
            else if (role == "employee")
            {
                isValidUser = data.Employees.Where(c => c.EmailId == email && c.Password == password).Count() > 0;
            }
            return isValidUser;
        }
       ...
       public int applyCourse(string email, int trainingId, string startDate, string endDate)
        {
            DateTime checkStartDate = DateTime.Parse(startDate);
            DateTime checkEndDate = DateTime.Parse(endDate);        
            ...
            if (trainingId != 0)
            {
                ...
                /*var datecClash = (from t in data.Training
                             ...
                             select t).Count();*/
                /*|| (e.t.Enddate <= checkEndDate && e.t.Enddate >= checkEndDate)*/


                var date = data.Training.Join(data.TrainingRequests, t => t.Id, tr => tr.TrainingId, (t, tr) => new { t, tr }).Where(e => e.tr.EmployeeEmail == email  && (e.t.Startdate >= checkStartDate && e.t.Enddate <= checkStartDate) ).Count(); 
